...
export abstract class AbstractHistoryChart implements OnInit {
  ...
  public forceLineChart : boolean = false; // oEMS
   ...
   * @param customShadow the customShadowColor if defined oEMS
   * @returns the backgroundColor and borderColor
   */
  public static getColors(color: string, chartType: 'line' | 'bar', customShadow: string ): { backgroundColor: string, borderColor: string} { // oEMS
    return {
      backgroundColor: customShadow ?? 'rgba(' + (chartType == 'bar' ? color.split('(').pop().split(')')[0] + ',0.4)' : color.split('(').pop().split(')')[0] + ',0.05)'), // oEMS
  ...
  public static getDataSet(element: HistoryUtils.DisplayValues, label: string, data: number[], stack: number, chartObject: HistoryUtils.ChartData, chartType: 'line' | 'bar'): Chart.ChartDataset {
    const dataset: Chart.ChartDataset = {
      ...
      ...AbstractHistoryChart.getColors(element.color, chartType, element.custom?.shadowcolor ?? null), // oEMS
      borderWidth: element.custom?.borderWidth ?? 2, // oEMS
    };
    return dataset;
  }
  ...
  protected loadChart() {
    ...
    if (ChronoUnit.isAtLeast(unit, ChronoUnit.Type.DAYS) && !this.forceLineChart) { // oEMS
      Promise.all([
  ...
  public static getOptions(chartObject: HistoryUtils.ChartData, chartType: 'line' | 'bar', service: Service,
    ...
    const showYAxisTitle: boolean = chartObject.yAxes.length > 0; // oEMS ALWAYS SHOW Y AXIS TITLE and do not hide
    ...
    options.plugins.tooltip.callbacks.label = (item: Chart.TooltipItem<any>) => {
      ...
      if(displayValue?.custom?.hideToolTipSuffix?? false){ // oEMS
  ...
  public static getYAxisOptions(options: Chart.ChartOptions, element: HistoryUtils.yAxes, translate: TranslateService, chartType: 'line' | 'bar', locale: string, showYAxisTitle?: boolean, data?: { min: number, max: number, stepSize: number }): Chart.ChartOptions {
    ...
    switch (element.unit) {
      ...
      case YAxisTitle.TEMPERATURE: // oEMS
      case YAxisTitle.OPENEMS_STATUS: // oEMS
      case YAxisTitle.ENUM: // oEMS
