 ...
 */
  public static calculateOtherConsumptionTotal(energyValues: QueryHistoricTimeseriesEnergyResponse, evcsComponents: EdgeConfig.Component[], consumptionMeterComponents: EdgeConfig.Component[]): number {
    ...
    consumptionMeterComponents.forEach(meter => {
      totalMeteredConsumption = this.addSafely(totalMeteredConsumption, energyValues.result.data[meter.id + '/ActiveConsumptionEnergy']); // Consumption instead of Production
    });
  ...
  public static calculateOtherConsumption(channelData: HistoryUtils.ChannelData, evcsComponents: EdgeConfig.Component[], consumptionMeterComponents: EdgeConfig.Component[]): number[] {
    ...
    evcsComponents.forEach(component => {
      channelData[component.id + '/ChargePower']?.forEach((value, index) => {
        totalEvcsConsumption[index] = this.addSafely(totalEvcsConsumption[index], value); // oEMS
      });
    });

    consumptionMeterComponents.forEach(meter => {
      channelData[meter.id + '/ActivePower']?.forEach((value, index) => {
        totalMeteredConsumption[index] = this.addSafely(totalMeteredConsumption[index], value); // oEMS
      });
    });
 ...
 */
  ...
  export type DisplayValues = {
    ...
    custom?: {
      ...
      formatNumber?: string,
      /**hide suffix in tooltip */ // oEMS
      hideToolTipSuffix?: boolean, // oEMS
      /**custom shadow color in rgba format */ // oEMS
      shadowcolor?: string,
      /** custom border width */ // oEMS
      borderWidth?: number
    },
    tooltip?: [{
 ...
 */
  ...
  export namespace ValueConverter {
    ...
    export const MAX_HUNDRED_MIN_ZERO = (value) => {
      if (value) {
    ...
    export const IGNORE_NULL_AND_LESS_OR_EQUAL_ZERO = (value) => {
        if(value > 0){
          return value;
        }
        return null;
    ...
    export const VALUES_GREATER_ZERO_EQUAL_TO_ONE = (value: number) => {
      if (value > 0) {
        return 1;
      } else {
        return null;
...
// oEMS Start
export namespace EvcsClusterUtils{
  export enum EvcsClusterState{
    Green = 0,
    Yellow = 1,
    Red = 2
  }

  export namespace StateConverter {
    export const GET_VALUE_OR_NULL_FROM_TARGET = (value: number, target: number) => {
      if(value == null || target == null || Number.isNaN(value) || Number.isNaN(target)){
        return null;
      }
      if (target - 1 < value && value <= target) {
        return value;
      }
      return null;
    };

    export const GET_NAME = (base: string, status: string) => {
      return base + " " + status;
    };

    export const IS_EXPECTED_STATE = (value: number, target: number) => {
      if(value == null || target == null || Number.isNaN(value) || Number.isNaN(target)){
        return false;
      }
      if (target - 1 < value && value <= target) {
  ...
  export enum ColorsAlpha{
    Green = 'rgba(85, 139, 47, 0.2)',
    Yellow = 'rgb(255, 152, 0, 0.2)',
    Red = 'rgb(244, 67, 54, 0.2)',
  }
}
// oEMS End
