...
// oEMS - START
export enum oemsWidgetFactory {
    'Controller.Evcs.Vehicle.Battery.Symmetric',
    'Controller.oems.CHP.SoC',
}
// oEMS - END
...
export enum WidgetNature {
    ...
    'io.openems.edge.thermometer.api.Thermometer', // oEMS
    'io.openems.edge.config.thermometer.ConfigThermometer', // oEMS
}
...
export class Widget {
    public name: WidgetNature | WidgetFactory | oemsWidgetFactory | string;
    ...
    public static parseWidgets(edge: Edge, config: EdgeConfig): Widgets {
        ...
        for (const nature of Object.values(WidgetNature).filter(v => typeof v === 'string')) {
            for (const componentId of config.getComponentIdsImplementingNature(nature.toString())) {
                ...
                if (nature === 'io.openems.edge.config.thermometer.ConfigThermometer' && list.some(e => e.name === 'io.openems.edge.config.thermometer.ConfigThermometer')) {
                    continue;
                }
                // oEMS END
                if (config.getComponent(componentId).isEnabled) { // oEMS
        ...
        // oEMS explicitely sort EVCS
        list.sort((w1, w2) => {
            if (w1.name === 'io.openems.edge.evcs.api.Evcs' && w2.name === 'io.openems.edge.evcs.api.Evcs') {
                const alias1: string = config.getComponentProperties(w1.componentId)['alias'];
                const alias2: string = config.getComponentProperties(w2.componentId)['alias'];
                if (alias1 && alias2) {
