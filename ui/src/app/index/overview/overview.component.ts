...
export class OverViewComponent implements OnInit, OnDestroy {
    ...
    public isAnonymized: boolean = false; // oEMS
    ...
    ngOnInit() {
        ...
        this.service.metadata.pipe(filter(metadata => !!metadata), take(1)).subscribe((metadata) => { // oEMS
            // oEMS START
            if (metadata.user.name.startsWith("_")) {
