  ...
  public static getChartData(config: EdgeConfig, translate: TranslateService): HistoryUtils.ChartData {
    ...
    const evcsComponents: EdgeConfig.Component[] = config.getComponentsImplementingNature("io.openems.edge.evcs.api.Evcs")
      .filter(component => !(
        ...
        config.getComponentsImplementingNature("io.openems.edge.evcs.api.MetaEvcs").includes(component) || // oEMS
        component.isEnabled === false)); // oEMS
    ...
    consumptionMeters.forEach(meter => {
      inputChannel.push({
        ...
        energyChannel: ChannelAddress.fromString(meter.id + '/ActiveConsumptionEnergy'), // oEMS using Consumption instead of Production
      });
    });
    ...
    return {
      ...
      output: (data: HistoryUtils.ChannelData) => {
        ...
        datasets.push({
          ...
          color: 'rgb(253,197,7)',
          stack: 0,
          hiddenOnInit: true, // oEMS
          noStrokeThroughLegendIfHidden: false, // oEMS
        });
        ...
        consumptionMeters.forEach((meter, index) => {
          datasets.push({
            name: meter.alias,
            nameSuffix: (energyValues: QueryHistoricTimeseriesEnergyResponse) => {
              return energyValues?.result.data[meter.id + '/ActiveConsumptionEnergy']; // oEMS Consumption instead of Production
            },
            converter: () => {
