        ...
        this.edge.sendRequest(this.websocket, new AppCenter.Request({
            ...
            this.registeredKeys.forEach(key => {
                ...
                (selectRegisteredKey.props.options as any[]).push({
                    value: key.keyId,
                    label: key.keyId,
                    description: key.keyDescription ? key.keyDescription : desc, // oEMS after some iterations: show app key, but description is either a more readable name or UI descr.
                });
            });
        }).catch(reason => {
            this.fields = this.getFields();
