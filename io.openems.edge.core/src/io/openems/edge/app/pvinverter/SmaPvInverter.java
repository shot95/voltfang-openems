...
public class SmaPvInverter extends AbstractOpenemsAppWithProps<SmaPvInverter, Property, Parameter.BundleParameter>
	...
	public static enum Property implements Type<Property, SmaPvInverter, Parameter.BundleParameter>, Nameable {
		...
		// oEMS Start
		INSTALL_SMART_HOME(AppDef.of(SmaPvInverter.class) //
				.setDefaultValue(false) //
				.setTranslatedLabel("App.IntegratedSystem.SMA.SmartEnergy.energymeter.available.label") //
				.setTranslatedDescription("App.IntegratedSystem.SMA.SmartEnergy.energymeter.available.description") //
				.setField(JsonFormlyUtil::buildCheckbox) //
		), //

		METER_ID(AppDef.componentId("meter0")), // -> SMART HOME METER + IP INPUT
	...
	protected ThrowingTriFunction<ConfigurationTarget, Map<Property, JsonElement>, Language, AppConfiguration, OpenemsNamedException> appPropertyConfigurationFactory() {
		return (t, p, l) -> {
			...
			// oEMS Start
			final var shouldInstallSH = this.getBoolean(p, Property.INSTALL_SMART_HOME);
			var gridMeterId = shouldInstallSH ? this.getId(t, p, Property.METER_ID)
					: "";
			// oEMS End

			final var components = Lists.newArrayList(// oEMS Lists.newArrayList
			...
			// oEMS Start
			if (shouldInstallSH) {
				components.add(new EdgeConfig.Component(
						// grid meter
						gridMeterId, "Sunny Home Manager 2.0", //
						"Meter.SMA.SHM20", JsonUtils.buildJsonObject() //
						.addPropertyIfNotNull("modbus.id", modbusId) //
						.addProperty("modbusUnitId", 3) //
