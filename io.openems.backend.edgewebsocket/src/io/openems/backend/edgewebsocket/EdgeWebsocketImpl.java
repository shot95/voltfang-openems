...
public class EdgeWebsocketImpl extends AbstractOpenemsBackendComponent
	...
	private void activate(Config config) {
		...
		if (this.metadata.isInitialized()) {
			// oEMS: Option A: Metadata is already initialized. We will not get an Metadata.Events.AFTER_IS_INITIALIZED later.
			this.startServer();
		}
	}
	...
	public void handleEvent(Event event) {
		...
		case Metadata.Events.AFTER_IS_INITIALIZED:
			// oEMS: Option B: Metadata initializes too late. We are already activated().
			this.startServer();
			break;
		}
	}
