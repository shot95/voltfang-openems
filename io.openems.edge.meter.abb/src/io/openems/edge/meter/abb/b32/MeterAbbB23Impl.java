...
public class MeterAbbB23Impl extends AbstractOpenemsMbusComponent
	...
	private void activate(ComponentContext context, Config config) {
		...
		super.activate(context, config.id(), config.alias(), config.enabled(), config.primaryAddress(), this.cm, "Mbus", // oEMS
				config.mbus_id());
		// register into mbus bridge task list
		this.getMBus().addTask(config.id(), new MbusTask(this.getMBus(), this)); // oEMS
	}
