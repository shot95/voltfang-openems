...
public class SumStateHandler implements Handler<SumStateMessage> {
	...
	public void send(ZonedDateTime sentAt, List<SumStateMessage> pack) {
		...
		var params = JsonUtils //
				.generateJsonArray(pack.stream() //
						.map(SumStateMessage::getParams) //
						.toList());
		if (!params.isEmpty()) {
	...
	private boolean isEdgeError(String edgeId) {
		...
		return sumState.filter(this::isSevere).isPresent(); // oEMS streamlined
	}
	...
	protected SumStateMessage getEdgeMessage(Edge edge, Level sumState) throws OpenemsException {
		...
		try {
			...
			return new SumStateMessage(edge.getId(), sumState, this.timeService.now(), sumStateSettings); // oEMS streamlined
