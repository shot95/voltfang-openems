...
public class ControllerIoHeatPumpSgReadyImpl extends AbstractOpenemsComponent
	...
	public void run() throws OpenemsNamedException {
		...
		case AUTOMATIC -> this.modeAutomatic(); //oEMS
		case MANUAL -> this.modeManual(); // oEMS
		}
	}
	...
	private void modeAutomatic() throws IllegalArgumentException, OpenemsNamedException {
		...
		final var soc = this.config.essType() == EssType.NO_ESS ? 100 : this.getEssSocOrZero(); // oEMS should be 100% to not lock the heatpump
	...
	private void modeManual() throws IllegalArgumentException, OpenemsNamedException {
		...
		switch (state) {
		case FORCE_ON -> this.forceOnState.switchOn(); // oEMS start
		case LOCK -> this.lockState.switchOn();
		case RECOMMENDATION -> this.recommState.switchOn();
		case REGULAR -> this.regularState.switchOn();
		case UNDEFINED -> this.regularState.switchOn(); // oEMS end
		}
	}

	private int getEssDischargePowerOrZero() {
		// oEMS config essType
		return switch (this.config.essType()) {
		case NO_ESS -> 0;
		case SYMMETRIC_ESS -> this.getChannelValueOrZeroAndSetStateChannel(this.sum.getEssActivePower(),
				this.getEssDischargePowerNotPresentChannel());
		case HYBRID_ESS -> this.getChannelValueOrZeroAndSetStateChannel(this.sum.getEssDischargePower(),
				this.getEssDischargePowerNotPresentChannel());
		};
	}
	...
	/*
	 * private int getEssSocOrHundred() { return
	 * this.getChannelValueOrHundretAndSetStateChannel(this.sum.getEssSoc(),
	 * this.getStateOfChargeNotPresentChannel()); }
	 */
	...
	// /**
	// * Get the Channel value or 100 if not present - Sets also the according state
	// * channel depending on the channel.
	// *
	// * @param value Channel value
	// * @param stateChannel Referring StateChannel that will be set if the value is
	...
	// * @return Current channel value as int.
	// */
	// private int getChannelValueOrHundretAndSetStateChannel(Value<Integer> value,
