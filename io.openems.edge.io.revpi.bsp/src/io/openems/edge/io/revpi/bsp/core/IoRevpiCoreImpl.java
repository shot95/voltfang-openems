...
public class IoRevpiCoreImpl extends AbstractOpenemsComponent
	...
	private static final int REBOOT_UPTIME_WINDOW_START = 10;
	private static final int REBOOT_UPTIME_WINDOW_END = 20;
	private static final int REBOOT_MIN_COUNT1 = 120;
	private static final int REBOOT_MIN_COUNT2 = 180;
	...
	private int cloudConnectionBrokenCnt = 0;
	...
	private void rebootCheck() {
		if (!this.config.reboot() || this.config.backendComponentId() == null
				|| this.config.backendComponentId().isBlank()) {
			return;
		}
		var cloudConnected = this.getCloudConnectionState();
		...
		if (cloudConnected == false //
				&& uptime.isDefined() //
				&& uptime.get() > REBOOT_UPTIME_WINDOW_START //
				&& uptime.get() < REBOOT_UPTIME_WINDOW_END) {
			if (this.cloudConnectionBrokenCnt++ > REBOOT_MIN_COUNT1) {
				this.logWarn(this.log, "Connection to Backend is still broken. Waiting for reboot trigger..");
			}
			if (this.cloudConnectionBrokenCnt > REBOOT_MIN_COUNT2) {
				this.reboot();
			}
		} else {
			this.cloudConnectionBrokenCnt = 0;
		}
	}

	private void reboot() {
		this.logWarn(this.log, "Rebooting system.");
		try {
			this.host.restartSystem();
		} catch (NotImplementedException nie) {
			this.logError(this.log, "Unable to reboot the system. Ex: " + nie.getMessage());
		}
	}
